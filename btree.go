...
type ItemIterator func(i []byte) bool
...
// forward.
func (s *items) insertAt(index int, item []byte) {
	*s = append(*s, nil)
	if index < len(*s) {
...
// back.
func (s *items) removeAt(index int) []byte {
	item := (*s)[index]
...
// index.
func (s items) find(item []byte) (index int, found bool) {
	i, j := 0, len(s)
	for i < j {
		h := i + (j-i)/2
		if !less(item, s[h]) {
...
func (n *node) insert(item []byte, maxItems int) []byte {
...
func (n *node) remove(item []byte, minItems int, typ toRemove) []byte {
	var i int
	var found bool
	switch typ {
	case removeMax:
...
// that we hit case A.
func (n *node) growChildAndRemove(i int, item []byte, minItems int, typ toRemove) []byte {
...
func (n *node) iterate(dir direction, start, stop []byte, includeStart bool, hit bool, iter ItemIterator) (bool, bool) {
	var ok bool
	switch dir {
	case ascend:
...
//
// BTree stores []byte instances in an ordered structure, allowing easy insertion,
...
func (t *BTree) ReplaceOrInsert(item []byte) []byte {
	if item == nil {
...
func (t *BTree) deleteItem(item []byte, typ toRemove) []byte {
...
func (t *BTree) Min() []byte {
	return min(t.root)
}

// Max returns the largest item in the tree, or nil if the tree is empty.
func (t *BTree) Max() []byte {
	return max(t.root)
}
...
func (c *Cursor) Seek(pivot []byte) []byte {
	c.stack = c.stack[:0]
	n := c.t.root
	for n != nil {
