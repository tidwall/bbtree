...
func all(t *BTree) (out [][]byte) {
	t.Ascend(func(a []byte) bool {
		out = append(out, a)
		return true
	})
	return
...
func allrev(t *BTree) (out [][]byte) {
	t.Descend(func(a []byte) bool {
		out = append(out, a)
		return true
	})
	return
...
func TestBTree(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if min, want := tr.Min(), []byte(Int(0)); bytes.Compare(min, want) != 0 {
			t.Fatalf("min: want %+v, got %+v", want, min)
		}
		if max, want := tr.Max(), []byte(Int(treeSize-1)); bytes.Compare(max, want) != 0 {
...
func ExampleBTree() {
	...
	fmt.Println("get3:      ", intString(tr.Get(Int(3))))
	fmt.Println("get100:    ", intString(tr.Get(Int(100))))
	fmt.Println("del4:      ", intString(tr.Delete(Int(4))))
	fmt.Println("del100:    ", intString(tr.Delete(Int(100))))
	fmt.Println("replace5:  ", intString(tr.ReplaceOrInsert(Int(5))))
	fmt.Println("replace100:", intString(tr.ReplaceOrInsert(Int(100))))
	fmt.Println("min:       ", intString(tr.Min()))
	fmt.Println("delmin:    ", intString(tr.DeleteMin()))
	fmt.Println("max:       ", intString(tr.Max()))
	fmt.Println("delmax:    ", intString(tr.DeleteMax()))
	fmt.Println("len:       ", tr.Len())
	// Output:
	// len:        10
	// get3:       3
...
func TestAscendRange(t *testing.T) {
	...
	var got [][]byte
	tr.AscendRange(Int(40), Int(60), func(a []byte) bool {
		got = append(got, a)
		return true
	})
...
func TestDescendRange(t *testing.T) {
	...
	var got [][]byte
	tr.DescendRange(Int(60), Int(40), func(a []byte) bool {
		got = append(got, a)
		return true
	})
...
func TestAscendLessThan(t *testing.T) {
	...
	var got [][]byte
	tr.AscendLessThan(Int(60), func(a []byte) bool {
		got = append(got, a)
		return true
	})
...
func TestDescendLessOrEqual(t *testing.T) {
	...
	var got [][]byte
	tr.DescendLessOrEqual(Int(40), func(a []byte) bool {
		got = append(got, a)
		return true
	})
	...
	got = got[:0]
	tr.DescendLessOrEqual(Int(60), func(a []byte) bool {
		if intInt(a) < 50 {
			return false
		}
		got = append(got, a)
...
func TestAscendGreaterOrEqual(t *testing.T) {
	...
	var got [][]byte
	tr.AscendGreaterOrEqual(Int(40), func(a []byte) bool {
		got = append(got, a)
		return true
	})
...
func TestDescendGreaterThan(t *testing.T) {
	...
	var got [][]byte
	tr.DescendGreaterThan(Int(40), func(a []byte) bool {
		got = append(got, a)
		return true
	})
	if want := rangrev(100)[:59]; !reflect.DeepEqual(got, want) {
...
func BenchmarkAscend(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := 0
		tr.Ascend(func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j++
			return true
		})
...
func BenchmarkDescend(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := len(arr) - 1
		tr.Descend(func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j--
...
func BenchmarkAscendRange(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := 100
		tr.AscendRange(Int(100), arr[len(arr)-100], func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j++
...
func BenchmarkDescendRange(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := len(arr) - 100
		tr.DescendRange(arr[len(arr)-100], Int(100), func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j--
...
func BenchmarkAscendGreaterOrEqual(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := 100
		k := 0
		tr.AscendGreaterOrEqual(Int(100), func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j++
...
func BenchmarkDescendLessOrEqual(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		j := len(arr) - 100
		k := len(arr)
		tr.DescendLessOrEqual(arr[len(arr)-100], func(item []byte) bool {
			if intInt(item) != intInt(arr[j]) {
				b.Fatalf("mismatch: expected: %v, got %v", intString(arr[j]), intString(item))
			}
			j--
...
func cloneTest(t *testing.T, b *BTree, start int, p [][]byte, wg *sync.WaitGroup, trees *[]*BTree) {
...
func TestCursor(t *testing.T) {
	...
	for item := c.First(); item != nil; item = c.Next() {
		a = append(a, fmt.Sprintf("%v", intInt(item)))
	}
	x := strings.Join(a, ",")
